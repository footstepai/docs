...
A list of matching places, each with coordinates, name, full label, distance from the search point in meters, and a confidence score. Results are sorted by proximity. The `query.near` field of the response shows the resolved coordinates so you can see what center was used.
