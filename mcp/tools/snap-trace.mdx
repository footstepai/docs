...
- `match_quality: "low"` — a flag saying the match isn't trustworthy.
- `match_quality_reason` — a short string explaining why the LLM should not quote distance/duration figures as ground truth.
- `terrain` is **omitted**. Ascent/descent/difficulty derived from a partial match disagree with reality often enough that surfacing them as authoritative numbers does more harm than good.

`distance_meters`, `duration_seconds`, and `edges` are still returned so you can inspect what was matched — they're useful diagnostics, just not numbers to quote. For high-confidence matches (≥ 0.5) none of these fields appear and the regular `terrain` block is included.
