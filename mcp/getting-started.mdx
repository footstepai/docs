...
If you're building a chat app that renders Footstep results on a map (deck.gl, MapLibre, Leaflet, Mapbox, Cesium…), the response is already shaped for you.

Every renderer-aware tool returns a `render` array of framework-agnostic GeoJSON layer descriptors — drop them straight into your map library, no decoder required:
...
See [render envelopes](/mcp/render-envelopes) for the full kind taxonomy and per-library examples.
...
See [response defaults & controls](/mcp/defaults) for the full flag matrix.
