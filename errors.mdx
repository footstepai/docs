...
The `error` field is a stable, machine-readable code you can match on in your code. The `message` field is a human-readable explanation that may change over time — don't parse it programmatically.
...
These indicate a problem on our side. These are usually transient — wait and retry.
